...
#
# `-f` as well as `-r`, because `-r` alone is true of a directory and grep's
# "Is a directory" goes down that same swallowed path to the same false zero.
[ -f "$log" ] && [ -r "$log" ] || { echo "count_tests.sh: cannot read $log" >&2; exit 1; }
