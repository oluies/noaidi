...
nothing itself, recording whichever version produced the goldens into
`reference/goldens/manifest.json` alongside schema, exported tables, sub-network
decomposition, LPF and optimisation results for each reference network. Prima itself is validated against ojAlgo and the Netlib
