...
| `network-lopf` | L2: linear optimal power flow, and unit commitment via Prima's branch-and-bound. |
| `network-pf` | L2: power flow — linear (one SPD solve per sub-network) and non-linear Newton-Raphson AC. No LP solver involved. |

353 tests pass in the aggregated build, plus 48 in the opt-in Netlib module.
