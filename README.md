...
break-even at 1e-9. One of those ten draws costs −621% at 1e-6 — −443% on the
other host this is measured on, since iteration counts on the largest dense
instances are platform-specific and NOTES gives both — and it is the one the
ladder happens to carry — which is how that number came to be reported
as a property of reduced precision rather than of the instance. Tracing it puts
...
iterations where a cold solve needs 35,392 for 1e-9 — the first digits are
nearly free and the last cost everything, so a float32 device removes only the
